...
hpc_date <- hpc_raw %>% mutate(FullDate = as.POSIXct(paste(hpc_raw$Date, hpc_raw$Time), format = "%d/%m/%Y %H:%M:%S")) %>% select(c('FullDate','Global_active_power'))
...
# Plot the data
plot(hpc_date$FullDate, hpc_date$Global_active_power, type = 'l', xlab = '', ylab = 'Global Active Power (kilowatts)')
